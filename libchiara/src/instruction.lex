%top{
	...
	#define YY_DECL int instruction_lex (XanHashTable *insts, XanHashTable *methods, XanHashTable *instIDs, XanHashTable *instructionToMethod)
	int instruction_lex (XanHashTable *insts, XanHashTable *methods, XanHashTable *instIDs, XanHashTable *instructionToMethod);
}
...
{INST_ID}		{
	if (instID == 0){
		...
		if (xanHashTable_lookup(insts, inst) == NULL){
			...
			if (instIDs != NULL){
				xanHashTable_insert(instIDs, (void *)(JITNUINT)instID, inst);
			}
			if (instructionToMethod != NULL){
				xanHashTable_insert(instructionToMethod, inst, currentMethod);
			}
		}
		currentMethod	= NULL;
...
[^\n ](.)+[a-zA-Z](.)+ {
	...
	if (	(methods != NULL)					&&
		(xanHashTable_lookup(methods, currentMethod) == NULL)	){
