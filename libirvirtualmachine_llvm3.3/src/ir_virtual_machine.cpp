...
public:
	...
	virtual void notifyObjectCompiled(const Module *M, const MemoryBuffer *obj) {
		...
		raw_fd_ostream IRObjectStream(objectFileName.c_str(), errStr, raw_fd_ostream::F_Binary);
...
static inline char * internal_linkerCommandLine (void);
...
void IRVM_finishPreCompilation (IRVM_t *self){
	 ...
	 */
	linkCommand	= internal_linkerCommandLine();
...
static inline void translate_ir_store_rel (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, t_llvm_function_internal *llvmFunction, BasicBlock *currentBB) {
	...
	llvmEa			= new IntToPtrInst(intLlvmEa, ptrValueToStoreType, "", currentBB);
...
static inline void translate_ir_load_rel (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, t_llvm_function_internal *llvmFunction, BasicBlock *currentBB) {
	...
	/* Fetch the base address				*/
	llvmBa			= get_LLVM_value_for_using(self, method, irBa, llvmFunction, currentBB);
	llvmBaType		= llvmBa->getType();

	/* Convert the base address to integer type 		*/
	intTypeToUseAsPointer		= get_LLVM_integer_type_to_use_as_pointer(self, llvmRoots);
	if (	(!IRMETHOD_hasAPointerType(irBa))	&&
		(!llvmBaType->isPointerTy())		){
		llvmBa			= new IntToPtrInst(llvmBa, get_LLVM_type(self, llvmRoots, IRMPOINTER, NULL), "", currentBB);
	}
	intLlvmBa		= new PtrToIntInst(llvmBa, intTypeToUseAsPointer, "", currentBB);

	/* Compute the effective address			*/
	if (	(IRDATA_isAVariable(irOffset))				||
		(IRDATA_isAConstant(irOffset) && ((irOffset->value).v != 0))	||
		(IRDATA_isASymbol(irOffset))					){

		/* Fetch the offset					*/
		llvmOffset		= get_LLVM_value(self, llvmRoots, method, irOffset, llvmFunction);
		if (((size_t)IRDATA_getSizeOfType(irOffset)) < sizeof(JITNUINT)){
			llvmOffset		= new ZExtInst(llvmOffset, intTypeToUseAsPointer, "", currentBB);
		}

		/* Add the offset to the base address to create the	*
		 * effective address					*/
		intLlvmEa		= BinaryOperator::Create(Instruction::Add, intLlvmBa, llvmOffset,  "", currentBB);
	...
	/* Convert the effective address to the appropriate LLVM*
	 * type							*/
	...
	llvmEa			= new IntToPtrInst(intLlvmEa, ptrReferentType, "", currentBB);
...
static inline char * internal_linkerCommandLine (void){
	...
	snprintf(linkCommand, linkCommandLength, "g++ -o %s.ildjit *.o %s %s %s -m32", programName, libraryDirs, librariesToStaticallyLink, librariesToDynamicallyLink);
