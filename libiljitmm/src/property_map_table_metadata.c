 ...
 */
/**
   *@file property_map_table_metadata.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
JITINT32 load_property_map_table (t_property_map_table *table, void *binary, JITUINT64 valid, JITUINT16 heap_size, JITINT8 *row_buf, JITINT8 *table_buf, JITUINT32 *bytes) {
    ...
    /* Create the table	*/
    PDEBUG("TABLE MANAGER:	Property map table\n");
    bytes_read      = 0;
    table->table    = (t_row_property_map_table **) sharedAllocFunction(sizeof(t_row_property_map_table *) * (table->cardinality));
    assert(table->table != NULL);

    /* Fill up the table	*/
    for (cardinality = 0; cardinality < (table->cardinality); cardinality++) {
        t_row_property_map_table *temp_row;
        temp_row                        = (t_row_property_map_table *) sharedAllocFunction(sizeof(t_row_property_map_table));
        ...
        if (    (((t_property_table *) private_get_table(NULL, PROPERTY_TABLE))->cardinality > METADATA_TABLE_LIMIT_16_BITS_ENCODING)) {
            read_from_buffer(table_buf, bytes_read, 4, &(temp_row->property_list));
            bytes_read += 4;
        } else {
            read_from_buffer(table_buf, bytes_read, 2, &(temp_row->property_list));
            bytes_read += 2;
        }
...
#ifdef PRINTDEBUG
    JITINT32 count;
    PDEBUG("			PROPERTY MAP TABLE	Cardinality = %d\n", cardinality);
    PDEBUG("Index		Parent		Property list\n");
    for (count = 0; count < cardinality; count++) {
        t_row_property_map_table *temp;
        temp = table->table[count];
        assert(temp != NULL);
        PDEBUG("%d		%d		%d\n", (count + 1), temp->parent, temp->property_list);
    }
    PDEBUG("\n");
...
void shutdown_property_map_table (t_property_map_table *table) {
    ...
    PDEBUG("TABLE MANAGER:		Property map shutting down\n");
