...
static inline void internal_sequentialCompilationPipeline (TranslationPipeline *pipe, Method method);
...
static XanStack		*methodsToCompile;
...
void SEQUENTIALPIPELINER_init (TranslationPipeline *pipeline) {
    ...
    /* Allocate the stack of methods to compile.
     */
    methodsToCompile	= xanStack_new(allocFunction, freeFunction, NULL);
...
static inline JITINT32 insert_method_into_pipeline_async (TranslationPipeline *pipe, Method method, JITFLOAT32 priority) {
    ...
    if (state != EXECUTABLE_STATE) {
        ...
        xanStack_push(methodsToCompile, method);
    }
...
static inline void internal_synchTillIRMethod (TranslationPipeline *pipe, Method method, JITFLOAT32 priority) {
    ...
    internal_insert_method_into_the_right_pipeline(pipe, method, priority, NULL, &notify, NULL, methodState, JITTRUE);
...
static inline void insert_method_into_pipeline (TranslationPipeline *pipe, Method method, JITFLOAT32 priority) {
    ...
    if (methodState != EXECUTABLE_STATE) {
        ...
        if (	(staticMemoryManager->compilationOfMethodLeadsToADeadlock(staticMemoryManager, currentThread, method))	&&
                (isMethodInPipe)											&&
                (isCCTORThread)												) {
            ...
            /* Insert the method into the rigth pipeline    */
            if (!isMethodInPipe) {
                internal_sequentialCompilationPipeline(pipe, method);

            } else if (!STATICMEMORY_isThreadTranslatingMethod(staticMemoryManager, currentThread, method)) {
...
static inline JITUINT32 DLA (t_ticket *ticket, XanVar *checkPoint) {
    ...
    if (    ((ildjitSystem->IRVM).behavior.aot)             &&
        ...
        while (item != NULL) {
            methodToCompile = (Method) xanList_getData(item);
...
static inline JITUINT32 iROptimizer (t_ticket *ticket, XanVar *checkPoint) {
    ...
    /* Optimize the method			*/
    state	= CODE_optimizeIR(&(ildjitSystem->codeGenerator), ticket->method, ticket->jobState, checkPoint);
...
static inline JITUINT32 cctorTranslator (t_ticket *ticket, XanVar *checkPoint) {
    ...
    toUnregister	= STATICMEMORY_registerTranslator(staticMemoryManager, method, currentThread);
...
static inline void internal_sequentialCompilationPipeline (TranslationPipeline *pipe, Method method) {
    ...
    /* Create the ticket					*/
    ticket			= internal_insertMethodIntoPipelineSet(pipe, method, NULL, NULL, NULL);
    ticket->runCCTORS	= JITTRUE;

    var			= xanVar_new(allocFunction, freeFunction);
    switch(method->getState(method)) {
        case CIL_STATE:

            /* Generate the IR					*/
            cilIRTranslator(ticket, var);

        case NEEDS_RECOMPILE_STATE:

            /* Optimize the IR					*/
            iROptimizer(ticket, var);

        case IR_STATE:

            /* Generate the machine code				*/
            iRMachineCodeTranslator(ticket, var);

        case MACHINE_CODE_STATE:

            /* Run the CCTORS					*/
            cctorTranslator(ticket, var);
            break;

        default:
            abort();
    }
    xanVar_destroyVar(var);
...
static inline void internal_destroyWorkersSet (workerSet_t *workers) {
    ...
    /* Destroy the queues.
     */
    xanQueue_destroyQueue(workers->pipe);
    xanQueue_destroyQueue(workers->busyWorkers);
}
...
static inline void internal_destroyThePipeline (TranslationPipeline *pipe) {
    ...
    xanStack_destroyStack(methodsToCompile);
...
static inline void internal_waitEmptyPipeline (TranslationPipeline *pipe) {
    ...
    /* Compile every cached methods.
     */
    while (xanStack_getSize(methodsToCompile) > 0) {
        Method	m;
        m	= xanStack_pop(methodsToCompile);
        if (m->getState(m) != EXECUTABLE_STATE) {
            insert_method_into_pipeline(pipe, m, MAX_METHOD_PRIORITY);
        }
    }
