...
static inline void translate_ir_fence (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, BasicBlock *currentBB);
...
void IRVM_finishPreCompilation (IRVM_t *self){
	...
	/* We have been configured to use the static compiler.
	 * LLVM 3.0 does not support the generation of machine code.
 	 */
		...
		for (JITUINT32 instPos=bb->startInst; instPos <= bb->endInst; instPos++){
			...
			switch (currentIRInstruction->type) {
				case IRCHECKNULL:
					...
					break;
                case IRFENCE:
					translate_ir_fence(self, llvmRoots, method, currentIRInstruction, llvmBB);
					break;
...
static inline void translate_ir_new_array (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, t_llvm_function_internal *llvmFunction, BasicBlock *currentBB) {
	...
	/* Create the function pointer				*/
	actualFunctionPtrConstant	= ConstantInt::get(intTypeToUseForPointers, (JITUINT64)(JITNUINT)self->gc->createArray);
	actualFunctionPtrValue		= ConstantExpr::getIntToPtr(actualFunctionPtrConstant, llvmRoots->newArrayFunctionType);
...
static inline void translate_ir_throw (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, t_llvm_function_internal *llvmFunction, BasicBlock *currentBB) {
	...
	if (catchInst == NULL){
		...
		actualFunctionPtrConstant	= ConstantInt::get(intTypeToUseForPointers, (JITUINT64)(JITNUINT)createOurException);
		actualFunctionPtrValue		= ConstantExpr::getIntToPtr(actualFunctionPtrConstant, llvmRoots->createExceptionFunctionType);
		exception			= CallInst::Create(actualFunctionPtrValue, llvmValue1, "", currentBB);

		/* Throw the exception.
		 */
		actualFunctionPtrConstant	= ConstantInt::get(intTypeToUseForPointers, (JITUINT64)(JITNUINT)_Unwind_RaiseException);
		actualFunctionPtrValue		= ConstantExpr::getIntToPtr(actualFunctionPtrConstant, llvmRoots->throwExceptionFunctionType);
...
static inline void translate_ir_free_object (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, t_llvm_function_internal *llvmFunction, BasicBlock *currentBB) {
	...
	actualFunctionPtrValue		= ConstantExpr::getIntToPtr(actualFunctionPtrConstant, llvmRoots->freeObjectFunctionType);
...
static inline void translate_ir_new_object (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, t_llvm_function_internal *llvmFunction, BasicBlock *currentBB) {
	...
	/* Create the function pointer				*/
	actualFunctionPtrConstant	= ConstantInt::get(get_LLVM_type(self, llvmRoots, IRUINT64, NULL), (JITUINT64)(JITNUINT)self->gc->allocObject);
	actualFunctionPtrValue		= ConstantExpr::getIntToPtr(actualFunctionPtrConstant, llvmRoots->allocObjectFunctionType);
...
static inline void translate_ir_memcpy_intrinsic (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, t_llvm_function_internal *llvmFunction, BasicBlock *currentBB) {
	...
	llvmMemcpyFunction->setAttributes(llvmMemcpyPAL);
	...
	llvmCallInst->setCallingConv(CallingConv::C);
	llvmCallInst->setTailCall(false);
	AttrListPtr llvmCallInst_PAL;
	llvmCallInst->setAttributes(llvmCallInst_PAL);
...
static inline void translate_ir_memset_intrinsic (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, t_llvm_function_internal *llvmFunction, BasicBlock *currentBB){
	...
	llvmMemsetFunction->setAttributes(llvmMemsetPAL);
...
static inline void translate_ir_fence (IRVM_t *self, IRVM_internal_t *llvmRoots, ir_method_t *method, ir_instruction_t *inst, BasicBlock *currentBB) {
    ...
    assert(IRDATA_isAConstant(par1));
    ...
    new FenceInst(llvmRoots->llvmModule->getContext(), ordering, CrossThread, currentBB);
