...
void IRVM_init(IRVM_t *self, t_running_garbage_collector *gc, IRVM_type * (*lookupValueType)(TypeDescriptor *type), ir_method_t * (*getIRMethod)(IR_ITEM_VALUE method), void (*leaveExecution)(JITINT32 exitCode), JITINT8 *leaveExecutionFunctionName, t_jit_function * (*getJITMethod)(IR_ITEM_VALUE method), JITINT32 (*recompilerFunction)(void *ilmethod, void *caller, ir_instruction_t *inst)){
	...
	MachineCodeGeneratorEventListener	*llvmListener;
	Type					*llvmPtrType;
	Type					*llvmIntType;

	/* Enable the LLVM exception handling mechanism		*/
	JITExceptionHandling	= true;

	/* Disable printing the machine code			*/
	...
	llvmListener		= new MachineCodeGeneratorEventListener(self);
	...
	/* Set the listener			*/
	vm->executionEngine->RegisterJITEventListener(llvmListener);
...
static inline void internal_createFunctionUtilities (IRVM_t *self, IRVM_internal_t *llvmRoots, JITINT8 *leaveExecutionFunctionName){
	...
	retType					= get_LLVM_type(self, llvmRoots, IRMPOINTER, NULL);
	functionType				= FunctionType::get(retType, argTypes, false);
	llvmRoots->createExceptionFunctionType	= PointerType::get(functionType, 0);
	...
	functionType				= FunctionType::get(retType, argTypes, false);
	llvmRoots->throwExceptionFunctionType	= PointerType::get(functionType, 0);

	/* Create the function to allocate arrays			*/
	argTypes.clear();
	argTypes.push_back(get_LLVM_type(self, llvmRoots, IRMPOINTER, NULL));
	argTypes.push_back(get_LLVM_type(self, llvmRoots, IRINT32, NULL));
	retType				= get_LLVM_type(self, llvmRoots, IROBJECT, NULL);
	functionType			= FunctionType::get(retType, argTypes, false);
	llvmRoots->newArrayFunctionType	= PointerType::get(functionType, 0);
	...
	functionType				= FunctionType::get(retType, argTypes, false);
	llvmRoots->freeObjectFunctionType	= PointerType::get(functionType, 0);

	/* Create the function to allocate objects			*/
	argTypes.clear();
	argTypes.push_back(get_LLVM_type(self, llvmRoots, IRMPOINTER, NULL));
	argTypes.push_back(get_LLVM_type(self, llvmRoots, IRUINT32, NULL));
	retType					= get_LLVM_type(self, llvmRoots, IROBJECT, NULL);
	functionType				= FunctionType::get(retType, argTypes, false);
	llvmRoots->allocObjectFunctionType	= PointerType::get(functionType, 0);
	...
	AttrListPtr llvmMemcmpPAL;
	{
		...
		llvmMemcmpPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->memcpyFunction->setAttributes(llvmMemcmpPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->memcpyFunction);
	...
	AttrListPtr llvmMemcpyPAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmMemcpyPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->memcpyFunction->setAttributes(llvmMemcpyPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->memcpyFunction);
	...
	AttrListPtr llvmMemsetPAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmMemsetPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->memsetFunction->setAttributes(llvmMemsetPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->memsetFunction);
	...
	AttrListPtr llvmStrchrPAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmStrchrPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->strchrFunction->setAttributes(llvmStrchrPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->strchrFunction);
	...
	AttrListPtr llvmStrcmpPAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmStrcmpPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->strcmpFunction->setAttributes(llvmStrcmpPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->strcmpFunction);
	...
	AttrListPtr llvmStrlenPAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmStrlenPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->strlenFunction->setAttributes(llvmStrlenPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->strlenFunction);
	...
	AttrListPtr llvmMallocPAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmMallocPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->mallocFunction->setAttributes(llvmMallocPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->mallocFunction);
	...
	AttrListPtr llvmCallocPAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmCallocPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->callocFunction->setAttributes(llvmCallocPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->callocFunction);
	...
	AttrListPtr llvmReallocPAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmReallocPAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->reallocFunction->setAttributes(llvmReallocPAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->reallocFunction);
	...
	AttrListPtr llvmFreePAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmFreePAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->freeFunction->setAttributes(llvmFreePAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->freeFunction);
	...
	AttrListPtr llvmCosh32PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmCosh32PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->cosh32Function->setAttributes(llvmCosh32PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->cosh32Function);
	...
	AttrListPtr llvmCosh64PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmCosh64PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->cosh64Function->setAttributes(llvmCosh64PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->cosh64Function);
	...
	AttrListPtr llvmAcos32PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmAcos32PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->acos32Function->setAttributes(llvmAcos32PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->acos32Function);
	...
	AttrListPtr llvmAcos64PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmAcos64PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->acos64Function->setAttributes(llvmAcos64PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->acos64Function);
	...
	AttrListPtr llvmFloor32PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmFloor32PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->floor32Function->setAttributes(llvmFloor32PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->floor32Function);
	...
	AttrListPtr llvmFloor64PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmFloor64PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->floor64Function->setAttributes(llvmFloor64PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->floor64Function);
	...
	AttrListPtr llvmIsnan32PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmIsnan32PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->isnan32Function->setAttributes(llvmIsnan32PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->isnan32Function);
	...
	AttrListPtr llvmIsnan64PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmIsnan64PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->isnan64Function->setAttributes(llvmIsnan64PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->isnan64Function);
	...
	AttrListPtr llvmIsinf32PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmIsinf32PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->isinf32Function->setAttributes(llvmIsinf32PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->isinf32Function);
	...
	AttrListPtr llvmIsinf64PAL;
	{
		...
		PAWI.Attrs = 0 | Attribute::NoUnwind;
		Attrs.push_back(PAWI);
		llvmIsinf64PAL = AttrListPtr::get(Attrs.begin(), Attrs.end());
	}
	llvmRoots->isinf64Function->setAttributes(llvmIsinf64PAL);
	llvmRoots->executionEngine->getPointerToFunction(llvmRoots->isinf64Function);
